...
import {
  HiHome, HiCube, HiTag, HiCollection, HiShoppingBag, HiUsers, HiStar,
  HiTicket, HiArchive, HiColorSwatch, HiTemplate, HiGlobe,
  HiCog, HiChartBar, HiX, HiLogout, HiBell,
  HiDocumentText, HiPhotograph, HiMail, HiCreditCard, HiBadgeCheck,
  HiLibrary,
} from 'react-icons/hi';
...
export default function Sidebar({ isOpen, onClose }) {
  ...
  const SidebarContent = () => (
    <div className="h-full flex flex-col">
      ...
      <div className="p-6 border-b border-gold/10 flex items-center justify-between">
        <div>
          <h2 className="text-xl font-playfair font-bold gold-text">Noor Mist</h2>
          <p className="text-xs text-gray-500 font-montserrat tracking-widest mt-0.5">ADMIN PANEL</p>
        </div>
        {onClose && (
      ...
      <nav className="flex-1 overflow-y-auto p-4 space-y-1">
        {navItems.map((item, i) =>
          item.separator ? (
            <div key={i} className="h-px bg-gray-800 my-3" />
          ) : (
            <NavLink
              key={item.path}
              to={item.path}
              ...
              onClick={onClose}
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-montserrat transition-all ${
                  isActive
      ...
      <div className="p-4 border-t border-gray-800">
        ...
        <button
          ...
          className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm text-gray-400 hover:text-red-400 hover:bg-red-500/5 transition-all"
        >
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex w-64 flex-shrink-0 bg-noir-card border-r border-gray-800 h-screen sticky top-0 flex-col">
        <SidebarContent />
      </aside>
        ...
        {isOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 h-full w-72 bg-noir-card border-r border-gray-800 z-50 lg:hidden"
            >
              <SidebarContent />
            </motion.aside>
          </>
