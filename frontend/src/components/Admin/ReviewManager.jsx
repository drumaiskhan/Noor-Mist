...
export default function ReviewManager({ review, onUpdate }) {
  ...
  const approve = useMutation({
    ...
    onSuccess: () => { toast.success('Review approved'); queryClient.invalidateQueries(['adminReviews']); },
  });

  const reject = useMutation({
    mutationFn: () => reviewAPI.updateStatus(review.id, 'rejected'),
    onSuccess: () => { toast.success('Review rejected'); queryClient.invalidateQueries(['adminReviews']); },
  });
