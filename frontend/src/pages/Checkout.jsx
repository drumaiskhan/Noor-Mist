                          ...
                          {bankAccounts.map(acc => (
                            <div key={acc.id} className="p-4 bg-noir rounded-xl border border-theme-border/60">
                              <div className="flex items-start gap-4">
                                {acc.logo_url && <img src={acc.logo_url} alt={acc.bank_name} className="w-12 h-12 object-contain rounded-lg bg-theme-surface p-1" />}
                                ...
                                {acc.qr_image_url && (
                                  <img src={acc.qr_image_url} alt="QR" className="w-20 h-20 object-contain rounded-lg border border-theme-border" />
                                )}
                              </div>
                            </div>
                          ))}
                          ...
                          {wallet ? (
                            <div className="flex items-start gap-4">
                              <div className="flex-1 space-y-1">
                                ...
                                <InfoRow label="Amount to Send" value={formatPrice(total)} highlight />
                                {wallet.instructions && <p className="text-xs text-theme-muted mt-2 italic">{wallet.instructions}</p>}
                              </div>
                              {wallet.qr_image_url && (
                                <img src={wallet.qr_image_url} alt="QR" className="w-24 h-24 object-contain rounded-lg border border-theme-border" />
                              )}
                            </div>
                          ) : (
                            <p className="text-theme-muted text-sm">Send <span className="text-gold font-bold">{formatPrice(total)}</span> to our {selectedMethod?.label} account. Contact us for details.</p>
                          )}
                        </div>
                      )}
                    ...
                    return (
                      <label
                        ...
                        className={`flex items-center gap-4 p-4 rounded-xl border cursor-pointer transition-all ${
                          form.paymentMethod === method.key ? 'border-gold bg-gold/5' : 'border-theme-border hover:border-theme-muted'
                        }`}
                      >
                        <input
                          type="radio" name="paymentMethod" value={method.key}
                          checked={form.paymentMethod === method.key}
                          onChange={handleChange} className="accent-gold"
                        />
                        {method.icon_url
                          ? <img src={method.icon_url} alt={method.label} className="w-7 h-7 object-contain" />
                          : <Icon className="w-6 h-6 text-gold flex-shrink-0" />
                        }
                        <div className="flex-1">
                          <p className="text-theme-text font-medium text-sm">{method.label}</p>
                          <p className="text-xs text-theme-muted">{method.description}</p>
                          {method.requires_proof && (
                            <p className="text-xs text-gold/70 mt-0.5">📎 Proof of payment required</p>
                          )}
                        </div>
                        {form.paymentMethod === method.key && <HiCheck className="w-5 h-5 text-gold flex-shrink-0" />}
                      </label>
                    );
                  })}
                </div>
