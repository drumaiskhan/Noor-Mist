...
function ContactEditor({ data, onSave }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <Field label="Email Address" value={form.email} onChange={f('email')} placeholder="contact@example.com" />
      ...
      <Field
        ...
        onChange={f('map_embed')}
        placeholder="An address (e.g. 123 Main St, Lahore) or a Maps embed src URL"
        hint={'Easiest: just type the address — it becomes a working embedded map automatically. If you paste a link from Google Maps\' "Share → Embed a map" option, use the src="..." URL from inside the embed code, not the page URL — a regular maps.google.com or maps.app.goo.gl link can\'t be embedded and will show blank.'}
      />
...
export default function PageEditor() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['page', activePage] }); toast.success('Page saved'); },
