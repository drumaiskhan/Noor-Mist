...
function PageSEOTab() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(['seoSettings']); toast.success('SEO updated'); setEditingPath(null); },
  });
...
function TrackingTab() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(['siteSettings']); toast.success('Tracking codes saved'); },
  });
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <button onClick={handleSave} className="btn-gold" disabled={saveMutation.isLoading}>
        <HiCheck className="w-4 h-4 inline mr-2" />
        {saveMutation.isLoading ? 'Saving…' : 'Save Tracking Codes'}
      </button>
    </div>
  );
}
...
function RobotsTab() {
  const queryClient = useQueryClient();
  const defaultRobots = `User-agent: *\nAllow: /\n\nDisallow: /admin/\nDisallow: /api/\nDisallow: /checkout\nDisallow: /account\n\nSitemap: https://noormist.com/sitemap.xml`;
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(['siteSettings']); toast.success('robots.txt saved'); },
  });
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      ...
      <button onClick={() => saveMutation.mutate()} className="btn-gold" disabled={saveMutation.isLoading}>
        <HiCheck className="w-4 h-4 inline mr-2" />
        Save robots.txt
      </button>
    </div>
