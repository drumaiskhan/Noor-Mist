...
import { productAPI, categoryAPI, collectionsAPI } from '../../services/api';
import {
  HiPlus, HiPencil, HiTrash, HiSearch, HiEye, HiEyeOff, HiStar, HiDownload, HiUpload, HiX, HiCheck,
} from 'react-icons/hi';
...
export default function Products() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['adminProducts', search, status, categoryId, collectionId, sort, page],
    queryFn: () => productAPI.getAll({
      search,
      status: status === 'all' ? '' : status,
      category_id: categoryId === 'all' ? '' : categoryId,
      collection_id: collectionId === 'all' ? '' : collectionId,
      sort,
      page,
      limit: 20,
    }),
  ...
  const { data: collectionsData } = useQuery({
    queryKey: ['collectionsForFilter'],
    queryFn: () => collectionsAPI.getAll(),
    staleTime: 5 * 60 * 1000,
  });
  const categories = categoriesData?.data?.categories || categoriesData?.data || [];
  const collections = collectionsData?.data?.collections || collectionsData?.data || [];
  ...
  const bulkMutation = useMutation({
    mutationFn: ({ ids, action }) => productAPI.bulkAction(ids, action),
    onSuccess: (_res, { action, ids }) => {
      queryClient.invalidateQueries({ queryKey: ['adminProducts'] });
      setSelectedIds([]);
      const labels = {
        delete: 'deleted', publish: 'published', unpublish: 'unpublished',
        feature: 'marked as featured', unfeature: 'removed from featured',
      };
      toast.success(`${ids.length} product${ids.length === 1 ? '' : 's'} ${labels[action] || 'updated'}`);
    },
  ...
  // Reset selection whenever the visible page/filters change so stale
  // ids from a previous page can't be silently bulk-acted on.
  useEffect(() => { setSelectedIds([]); }, [search, status, categoryId, collectionId, sort, page]);
  ...
  const handleBulk = (action) => {
    ...
    if (action === 'delete' && !window.confirm(`Delete ${selectedIds.length} selected product(s)? This cannot be undone.`)) {
      return;
    }
      ...
      <AnimatePresence>
        {selectedIds.length > 0 && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -8 }}
            className="flex flex-wrap items-center gap-3 bg-gold/10 border border-gold/30 rounded-xl px-4 py-3"
          >
            <span className="text-sm text-gold font-medium">{selectedIds.length} selected</span>
            <div className="flex flex-wrap items-center gap-2 ml-auto">
              <button
                onClick={() => handleBulk('publish')}
                disabled={bulkMutation.isPending}
                className="inline-flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg border border-gray-600 text-gray-300 hover:border-green-400 hover:text-green-400 transition-colors disabled:opacity-50"
              >
                <HiEye className="w-3.5 h-3.5" /> Publish
              </button>
              <button
                onClick={() => handleBulk('unpublish')}
                disabled={bulkMutation.isPending}
                className="inline-flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg border border-gray-600 text-gray-300 hover:border-yellow-400 hover:text-yellow-400 transition-colors disabled:opacity-50"
              >
                <HiEyeOff className="w-3.5 h-3.5" /> Unpublish
              </button>
              <button
                onClick={() => handleBulk('feature')}
                disabled={bulkMutation.isPending}
                className="inline-flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg border border-gray-600 text-gray-300 hover:border-gold hover:text-gold transition-colors disabled:opacity-50"
              >
                <HiStar className="w-3.5 h-3.5" /> Feature
              </button>
              <button
                onClick={() => handleBulk('unfeature')}
                disabled={bulkMutation.isPending}
                className="inline-flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg border border-gray-600 text-gray-300 hover:border-gray-400 transition-colors disabled:opacity-50"
              >
                <HiOutlineStar className="w-3.5 h-3.5" /> Unfeature
              </button>
              <button
                onClick={() => handleBulk('delete')}
                disabled={bulkMutation.isPending}
                className="inline-flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg border border-gray-600 text-gray-300 hover:border-red-400 hover:text-red-400 transition-colors disabled:opacity-50"
              >
                <HiTrash className="w-3.5 h-3.5" /> Delete
              </button>
              <button
                onClick={() => setSelectedIds([])}
                className="inline-flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg text-gray-400 hover:text-white transition-colors"
              >
              ...
              {isLoading ? (
                [...Array(5)].map((_, i) => (
                  <tr key={i} className="border-b border-gray-800/50">
                    <td className="p-4"><div className="skeleton h-5 w-5 rounded" /></td>
                ...
                products.map((product, index) => (
                  <motion.tr
                    ...
                    className={`border-b border-gray-800/50 hover:bg-gold/5 transition-colors ${selectedIds.includes(product.id) ? 'bg-gold/5' : ''}`}
                  >
                    <td className="p-4">
                      <input
                        type="checkbox"
