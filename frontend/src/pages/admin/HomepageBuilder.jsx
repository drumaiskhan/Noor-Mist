...
function BrandingTab() {
  ...
  const updateThemeMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(['activeTheme']);
  ...
  return (
    <div className="space-y-8 max-w-2xl">
      ...
      <div className="luxury-card p-6 space-y-4">
        ...
        <button onClick={handleSaveColors} className="btn-gold w-full" disabled={updateThemeMutation.isLoading}>
          <HiCheck className="w-4 h-4 inline mr-2" />
          {updateThemeMutation.isLoading ? 'Saving…' : 'Save Colors'}
        </button>
      </div>
      ...
      <div className="luxury-card p-6 space-y-4">
        ...
        <button onClick={handleSaveBranding} className="btn-gold w-full" disabled={updateSettingsMutation.isLoading}>
          <HiCheck className="w-4 h-4 inline mr-2" />
          {updateSettingsMutation.isLoading ? 'Saving…' : 'Save Branding'}
        </button>
      </div>
    </div>
  );
...
function FontsTab() {
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      ...
      <button onClick={handleSave} className="btn-gold" disabled={updateMutation.isLoading}>
        <HiCheck className="w-4 h-4 inline mr-2" />
        {updateMutation.isLoading ? 'Saving…' : 'Save Fonts'}
      </button>
    </div>
  );
}
...
export default function HomepageBuilder() {
  ...
  const reorderMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries(['homepageSections']),
  });
