...
export default function PaymentShipping() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(['siteSettings']); toast.success('Settings saved'); },
  });
        ...
        <motion.div key={tab} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.15 }}>
          ...
          {tab === 'Payment Methods' && (
            <div className="space-y-4 max-w-2xl">
              ...
              <button onClick={savePayment} className="btn-gold" disabled={saveMutation.isLoading}>
                <HiCheck className="w-4 h-4 inline mr-2" />{saveMutation.isLoading ? 'Saving…' : 'Save Payment Settings'}
              </button>
            </div>
          )}
          ...
          {tab === 'Shipping Zones' && (
            <div className="space-y-4 max-w-2xl">
              ...
              <button onClick={saveShipping} className="btn-gold" disabled={saveMutation.isLoading}>
                <HiCheck className="w-4 h-4 inline mr-2" />{saveMutation.isLoading ? 'Saving…' : 'Save Shipping Settings'}
              </button>
            </div>
          )}
        </motion.div>
