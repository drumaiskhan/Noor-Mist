...
const linkTypeFromLink = (link, collections) => {
  ...
  if (
    match &&
    collections.some((collection) => collection.slug === match[1])
  ) {
    return "collection";
  }
...
// Convert DB timestamp into datetime-local input format.
...
// Convert datetime-local value into UTC ISO string.
...
export default function Announcements() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      ...
      toast.error(
        error.response?.data?.message ||
          "Failed updating announcement status"
      );
    },
  });
  ...
  const handleSubmit = () => {
    ...
    if (
      ...
      toast.error(
        "Select a collection for the Shop Now button"
      );
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-400 mt-2">
            Manage the homepage promo banner — shown above the hero,
            swipeable if you add more than one. (Looking for the thin
            scrolling text bar at the very top of the site instead? That's
            the "Announcement Bar" under Settings.)
          </p>
        </div>
      ...
      {showForm && (
        ...
        >
          ...
          <p className="text-xs text-gray-500 mb-4">
            Leave either date blank for no limit.
            Outside this window the banner
            automatically stops showing on the
            homepage.
          </p>
