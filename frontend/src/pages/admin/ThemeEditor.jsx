              ...
              {themes.length === 0 ? (
                ...
                themes.map((theme) => (
                  ...
                  >
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {activeTheme?.id !== theme.id && (
                        <button onClick={() => applyMutation.mutate(theme.id)} className="btn-gold !py-1.5 !px-3 !text-xs" disabled={applyMutation.isLoading}>
                          Apply
                        </button>
                      )}
                      <button onClick={() => openTheme(theme)} className="btn-outline !py-1.5 !px-3 !text-xs">Customize</button>
                      <button onClick={() => handleDuplicate(theme)} className="btn-outline !py-1.5 !px-3 !text-xs" disabled={duplicateMutation.isLoading}>Duplicate</button>
                      ...
                      <button
                        ...
                        disabled={deleteMutation.isLoading || themes.length <= 1}
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="theme-card p-6">
          ...
          <div className="flex gap-4 mt-8 flex-wrap pt-6 border-t border-theme-border">
            ...
            <button
              ...
              disabled={saving || applyMutation.isLoading}
            >
              Save &amp; Apply
            </button>
          </div>
