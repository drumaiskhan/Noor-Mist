...
import {
  ...
  HiChevronRight, HiChevronDown, HiArchive, HiCheck,
} from 'react-icons/hi';
                    ...
                    <div className="space-y-4">
                      {orders.map((order) => (
                        <div key={order.id} className="border border-theme-border/60 rounded-xl p-4 hover:border-gold/30 transition-colors">
                          ...
                          >
                            <div className="flex items-start justify-between gap-4 mb-3">
                              <div className="flex items-center gap-2">
                                <HiChevronDown className={`w-4 h-4 text-theme-muted flex-shrink-0 transition-transform ${expandedOrderId === order.id ? 'rotate-180' : ''}`} />
                                <div>
                          ...
                          <AnimatePresence>
                            {expandedOrderId === order.id && <OrderTracking orderId={order.id} />}
                          </AnimatePresence>
                        </div>
                      ))}
                    </div>
