...
import { productAPI, reviewAPI, settingsAPI, productPageSectionsAPI } from '../services/api';
...
const TAB_KEYS = ['description', 'notes', 'details', 'reviews'];
const TAB_LABELS = {
...
// Used only until the real product_page_sections config loads (or if that
// table is ever empty) — same order/visibility this page always had.
const DEFAULT_PAGE_SECTIONS = [
  { section_key: 'trust_badges', is_enabled: true, position: 3 },
  { section_key: 'description', is_enabled: true, position: 4 },
  { section_key: 'notes', is_enabled: true, position: 5 },
  { section_key: 'details', is_enabled: true, position: 6 },
  { section_key: 'reviews', is_enabled: true, position: 7 },
  { section_key: 'related', is_enabled: true, position: 8 },
];
...
export default function ProductDetail() {
  ...
  const freeShippingThreshold = settingsData?.shipping_free_threshold !== undefined
    ? Number(settingsData.shipping_free_threshold) : 5000;

  // Admin-configurable product page layout (Admin → Product Page Builder).
  // Falls back to the historical fixed order/visibility if the section
  // config hasn't loaded yet or the table is empty, so the page never
  // breaks or blanks out while this is in flight.
  const { data: sectionsData } = useQuery({
    queryKey: ['productPageSections'],
    queryFn: () => productPageSectionsAPI.getSections(),
    staleTime: 5 * 60 * 1000,
  });
  const pageSections = sectionsData?.data?.length ? sectionsData.data : DEFAULT_PAGE_SECTIONS;
  const isSectionEnabled = (key) => {
    const s = pageSections.find((sec) => sec.section_key === key);
    return s ? s.is_enabled : true;
  };
  const tabsConfig = TAB_KEYS
    .map((key) => ({ key, label: TAB_LABELS[key], ...(pageSections.find((s) => s.section_key === key) || {}) }))
    .filter((s) => s.is_enabled !== false)
    .sort((a, b) => (a.position ?? 0) - (b.position ?? 0));
  const showTrustBadges = isSectionEnabled('trust_badges');
  const showRelated = isSectionEnabled('related');
  ...
  // If the admin disables whichever tab is currently active (or on first
  // load), fall back to the first tab that's actually enabled.
  useEffect(() => {
    if (tabsConfig.length && !tabsConfig.some((t) => t.key === activeTab)) {
      setActiveTab(tabsConfig[0].key);
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-playfair font-bold mb-4">{t.product_not_found_title}</h2>
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-32 md:pb-16">
        ...
        {/* Tabs — order and visibility are admin-configurable
            (Admin → Product Page Builder) */}
        {tabsConfig.length > 0 && (
        <div className="mt-16">
          {/* Tab Navigation */}
          <div className="flex overflow-x-auto scrollbar-hide border-b border-gold/10 mb-8">
            {tabsConfig.map((tab) => (
              <button
                key={tab.key}
                onClick={() => setActiveTab(tab.key)}
                className={`flex-shrink-0 whitespace-nowrap px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm font-montserrat uppercase tracking-wider transition-all border-b-2 -mb-[2px] ${
                  activeTab === tab.key
                    ? 'text-gold border-gold'
                    : 'text-theme-muted border-transparent hover:text-theme-text'
                }`}
              >
                {tab.key === 'reviews' ? `${tab.label} (${product.review_count || 0})` : tab.label}
              </button>
            ))}
          </div>
