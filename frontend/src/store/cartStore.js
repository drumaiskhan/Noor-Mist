...
const getCurrency = () => localStorage.getItem('noor_mist_currency') || '₨';
  ...
  persist(
    (set, get) => ({
      ...
      applyCoupon: async (code) => {
        ...
        try {
          ...
          if (subtotal < parseFloat(coupon.minimum_order)) {
            return {
              success: false,
              message: `Minimum order of ${getCurrency()}${Number(coupon.minimum_order).toLocaleString()} required`,
            };
          }
          ...
          set({ appliedCoupon: coupon, couponDiscount: finalDiscount });
          return { success: true, message: `Coupon applied! You saved ${getCurrency()}${Math.round(finalDiscount).toLocaleString()}` };
        } catch (err) {
      ...
      _recalculateCouponDiscount: () => {
        ...
        if (appliedCoupon.max_discount != null) discount = Math.min(discount, Number(appliedCoupon.max_discount));
