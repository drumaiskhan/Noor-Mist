...
const useAuthStore = create((set) => ({
  ...
  checkAuth: () => {
    ...
    set({ token, user, isAuthenticated: !!token && !!user, isAdmin: user?.role === 'admin' });
  },
  ...
  login: async (email, password) => {
    ...
    try {
      ...
      set({ user: data.user, token: data.token, isAuthenticated: true, isAdmin: data.user?.role === 'admin', isLoading: false });
      return data;
    } catch (error) {
  ...
  verifyEmailOtp: async (payload) => {
    set({ isLoading: true, error: null });
    try {
      const { data } = await authAPI.verifyEmailOtp(payload);
      localStorage.setItem('noor_mist_token', data.token);
      localStorage.setItem('noor_mist_user', JSON.stringify(data.user));
      set({ user: data.user, token: data.token, isAuthenticated: true, isAdmin: data.user?.role === 'admin', isLoading: false });
      return data;
