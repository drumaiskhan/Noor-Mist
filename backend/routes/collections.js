...
router.get('/', async (req, res) => {
  try {
    const homepageOnly = String(req.query.homepage || '').toLowerCase() === 'true';
    const result = await query(
      homepageOnly
        ? 'SELECT * FROM collections WHERE is_active=true AND show_on_homepage=true ORDER BY name ASC'
        : 'SELECT * FROM collections WHERE is_active=true ORDER BY name ASC'
    );
