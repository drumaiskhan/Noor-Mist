...
function buildProductQuery(params, isAdmin = false) {
  // Admin product list needs to see drafts (is_visible=false) too, so the
  // storefront-only visibility filter is skipped for admins and replaced
  // with an explicit status filter instead (both branches derive from the
  // same is_visible column — there's no separate "archived" state yet).
  const conditions = isAdmin ? [] : ['p.is_visible = true'];
  const values = [];
  let idx = 1;

  if (isAdmin && params.status) {
    if (params.status === 'published') conditions.push('p.is_visible = true');
    else if (params.status === 'draft' || params.status === 'archived') conditions.push('p.is_visible = false');
  }
...
// POST /api/products/bulk (admin) — bulk actions for the admin product list.
// Body: { ids: [1,2,3], action: 'delete' | 'publish' | 'unpublish' | 'feature' | 'unfeature' }
router.post('/bulk', requireAdmin, async (req, res) => {
  try {
    ...
    const cleanIds = ids.map((id) => parseInt(id, 10)).filter((id) => Number.isInteger(id));
    if (!cleanIds.length) return res.status(400).json({ error: 'No valid ids provided' });

    const ACTIONS = {
      delete: { sql: 'DELETE FROM products WHERE id = ANY($1::int[])' },
      publish: { sql: 'UPDATE products SET is_visible = true, updated_at = NOW() WHERE id = ANY($1::int[])' },
      unpublish: { sql: 'UPDATE products SET is_visible = false, updated_at = NOW() WHERE id = ANY($1::int[])' },
      feature: { sql: 'UPDATE products SET is_featured = true, updated_at = NOW() WHERE id = ANY($1::int[])' },
      unfeature: { sql: 'UPDATE products SET is_featured = false, updated_at = NOW() WHERE id = ANY($1::int[])' },
    };
    ...
    const result = await query(chosen.sql, [cleanIds]);
    res.json({ message: `Bulk ${action} applied`, affected: result.rowCount ?? cleanIds.length });
  } catch (error) {
