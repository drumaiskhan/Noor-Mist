...
async function initDatabase() {
  try {
    ...
    await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS marketing_opt_in BOOLEAN DEFAULT true`);
    ...
    // ── Card payments (Safepay) migration ──────────────────────────────────
    // Existing databases are never dropped — these are additive columns only,
    // so upgrading to card payments never touches existing order data.
    await query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS payment_provider VARCHAR(30)`);
    await query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS payment_tracker VARCHAR(100)`);
    await query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS payment_transaction_id VARCHAR(100)`);
    await query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS payment_paid_at TIMESTAMP`);
    await query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS payment_token_hash VARCHAR(255)`);
    await query(`CREATE INDEX IF NOT EXISTS idx_orders_payment_tracker ON orders(payment_tracker)`);
    // 'pending_payment' already exists in the status check above; card orders
    // start there while the shopper is on the Safepay hosted page and only
    // move to 'confirmed' once the payment is verified.
    ...
    // Remove the old passwordless-login template from earlier builds; the storefront
    // now uses email verification on first registration and normal password login.
    await query(`DELETE FROM email_templates WHERE key = 'login_link'`).catch(() => {});

    // First-time email verification for customer accounts. Existing accounts are
    // treated as verified so a migration never locks out current customers.
    await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS email_verified BOOLEAN DEFAULT true`);
    await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS email_verification_token_hash VARCHAR(255)`);
    await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS email_verification_expires TIMESTAMP`);
    await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS email_verification_otp_hash VARCHAR(255)`);
    await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS email_verification_otp_expires TIMESTAMP`);
    await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS email_verification_otp_attempts INTEGER DEFAULT 0`);

    // Password reset — token + expiry on users (migration for existing DBs)
    ...
    // Password reset via OTP — a 6-digit code emailed alongside the reset
    // link, for people who'd rather type a code than click a link. Stored
    // as a bcrypt hash (like the password itself) rather than plaintext,
    // with an attempt counter so a leaked/guessed code can't be brute-forced.
    ...
    await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS login_token_hash VARCHAR(255)`);
    await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS login_token_expires TIMESTAMP`);

    // Coupon management migrations — keep existing production databases compatible
    // with the richer admin coupon editor without requiring a destructive reset.
    await query(`ALTER TABLE coupons ADD COLUMN IF NOT EXISTS max_discount DECIMAL(10,2)`);
    await query(`ALTER TABLE coupons ADD COLUMN IF NOT EXISTS description TEXT`);
    await query(`ALTER TABLE coupons ADD COLUMN IF NOT EXISTS starts_at TIMESTAMP`);

    // Customer order confirmation link
    await query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS confirmation_token_hash VARCHAR(255)`);
    await query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS confirmation_token_expires TIMESTAMP`);

    // Newsletter subscribers — real storage to replace the frontend-only fake form
    await query(`
    ...
    if (!adminCheck.rows.length) {
      ...
      await query(
        "INSERT INTO users (email, password_hash, first_name, last_name, role, email_verified) VALUES ($1, $2, $3, $4, 'admin', true)",
        [process.env.ADMIN_EMAIL || 'admin@example.com', hash, 'Admin', 'Noor Mist']
      );
      console.log(`Default admin created: ${process.env.ADMIN_EMAIL || 'admin@example.com'} / admin123`);
    }

    // Seed initial collections only when the store has none. These are normal
    // database records, so admins can rename, replace images, disable, or delete
    // them from Admin > Collections without touching frontend code.
    const collectionCheck = await query('SELECT COUNT(*)::int AS count FROM collections');
    if (collectionCheck.rows[0].count === 0) {
      const initialCollections = [
        ['Men\'s Fragrances', 'Bold and sophisticated scents for the modern gentleman', 'https://images.unsplash.com/photo-1523293182086-7651a899d37f?w=800&q=80', 'male'],
        ['Women\'s Fragrances', 'Elegant and captivating perfumes for the modern woman', 'https://images.unsplash.com/photo-1595535873420-a599195b3f4a?w=800&q=80', 'female'],
        ['Oud Collection', 'Rich, intense Arabian oud blended with exotic notes', 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800&q=80', 'unisex'],
        ['Royal Collection', 'Prestigious and luxurious signature fragrances', 'https://images.unsplash.com/photo-1541643600914-78b084683702?w=800&q=80', 'unisex'],
        ['Limited Edition', 'Exclusive seasonal releases in limited quantities', 'https://images.unsplash.com/photo-1587834685524-2ac8b5578f1b?w=800&q=80', 'unisex'],
        ['Gift Sets', 'Curated fragrance sets for special occasions', 'https://images.unsplash.com/photo-1549465220-1a8b9238cd48?w=800&q=80', 'unisex'],
      ];
      for (const [name, description, image_url, gender] of initialCollections) {
        const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
        await query(
          `INSERT INTO collections (name, slug, description, image_url, gender, show_on_homepage, is_active)
           VALUES ($1,$2,$3,$4,$5,true,true) ON CONFLICT (slug) DO NOTHING`,
          [name, slug, description, image_url, gender]
        );
      }
    }
    ...
    const sections = [
      ...
      { type: 'trust_badges', title: 'Trust Badges', position: 3, data: { freeShippingTitle: 'Free Shipping', freeShippingText: 'On orders over {{free_shipping_threshold}}', authenticTitle: '100% Authentic', authenticText: 'Guaranteed original fragrances', returnsTitle: 'Easy Returns', returnsText: '7-day hassle-free returns', supportTitle: '24/7 Support', supportText: 'Dedicated customer service' } },
      { type: 'bestsellers', title: 'Best Sellers', position: 4, data: { title: 'Best Sellers', subtitle: 'Our most loved fragrances by customers' } },
      { type: 'new_arrivals', title: 'New Arrivals', position: 5, data: { title: 'New Arrivals', subtitle: 'Fresh from our atelier' } },
      { type: 'perfume_finder', title: 'Perfume Finder', position: 6, data: { title: 'Find Your Signature Scent', subtitle: 'Answer a few questions and discover your perfect fragrance' } },
      { type: 'brand_story', title: 'Brand Story', position: 7, data: { title: 'The Art of Perfumery', body: 'Noor Mist was born from a passion for luxury and the timeless art of perfumery. Each fragrance is a masterpiece, crafted with the rarest ingredients sourced from around the world.', image: 'https://images.unsplash.com/photo-1595535873420-a599195b3f4a?w=800&q=80' } },
      { type: 'testimonials', title: 'Testimonials', position: 8, data: { title: 'What Our Customers Say' } },
      { type: 'instagram', title: 'Instagram Feed', position: 9, data: { title: 'Follow Our Journey', handle: '@noormist' } },
      { type: 'newsletter', title: 'Newsletter', position: 10, data: { title: 'Join the Noor Mist Family', subtitle: 'Subscribe and receive 10% off your first order plus exclusive access to new launches.' } },
    ];
    ...
    // Seed product detail page sections (admin-configurable layout)
    const productPageSections = [
      { key: 'gallery', label: 'Image Gallery', position: 1 },
      { key: 'info', label: 'Title, Price & Buy Box', position: 2 },
      { key: 'trust_badges', label: 'Shipping & Trust Badges', position: 3 },
      { key: 'description', label: 'Description Tab', position: 4 },
      { key: 'notes', label: 'Fragrance Notes Tab', position: 5 },
      { key: 'details', label: 'Details Tab', position: 6 },
      { key: 'reviews', label: 'Reviews Tab', position: 7 },
      { key: 'related', label: 'Related Products', position: 8 },
    ];
    for (const s of productPageSections) {
      await query(
        `INSERT INTO product_page_sections (section_key, label, position, is_enabled)
         VALUES ($1, $2, $3, true)
    ...
    const defaultSettings = [
      ...
      { key: 'delivery_estimate', value: '3–4 working days' },
      { key: 'email_test_subject', value: '{{store_name}} — Email Configuration Test' },
      { key: 'email_test_body', value: '<h1 style="color:#1a1a1a;font-family:Georgia,serif;">Email Delivery Confirmed</h1><p>This is a test email sent from {{store_name}}.</p><p><strong>Provider:</strong> {{provider}}</p><p><strong>From:</strong> {{from_address}}</p><p><strong>Sent to:</strong> {{recipient_email}}</p><p><strong>Sent at:</strong> {{sent_at}}</p><p>If this arrived correctly, your email delivery is working.</p>' },
      { key: 'email_test_footer', value: 'This is an automated test message and does not require a reply.' },
    ];
