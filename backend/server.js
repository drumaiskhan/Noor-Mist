...
app.use(
  express.json({
    limit: '10mb',
    // Capture the exact raw bytes alongside the parsed body so webhook
    // handlers (Safepay card payments) can verify HMAC signatures against
    // the untouched payload — signature checks fail if even whitespace
...
app.use('/api/product-page-sections', require('./routes/productPageSections'));
